...
fn G(ref v_a: u64,ref v_b: u64,ref v_c: u64,ref v_d: u64, x: u64, y: u64) {
    ...
    // 2^32 = 4294967296
    v_d = (v_d_xor_v_a/4294967296) | (TryInto::<u128,u64>::try_into((Into::<u64,u128>::into(v_d_xor_v_a) * 4294967296) & BoundedInt::<u64>::max().into()).unwrap());
    
    v_c = u64_wrapping_add(v_c, v_d);

    let v_b_xor_v_c = v_b ^ v_c;
    // 2^24 = 16777216, 2^40 = 1099511627776
    v_b = (v_b_xor_v_c/16777216) | (TryInto::<u128,u64>::try_into((Into::<u64,u128>::into(v_b_xor_v_c) * 1099511627776) & BoundedInt::<u64>::max().into()).unwrap());

    v_a = u64_wrapping_add(v_a, u64_wrapping_add(v_b, y));

    let v_d_xor_v_a = v_d ^ v_a;
    // 2^16 = 65536, 2^48 = 281474976710656
    v_d = (v_d_xor_v_a/65536) | (TryInto::<u128,u64>::try_into((Into::<u64,u128>::into(v_d_xor_v_a) * 281474976710656) & BoundedInt::<u64>::max().into()).unwrap());

    v_c = u64_wrapping_add(v_c, v_d);

    let v_b_xor_v_c = v_b ^ v_c;
    // 2^63 = 9223372036854775808, 2^1 = 2
    v_b = (v_b_xor_v_c/9223372036854775808) | (TryInto::<u128,u64>::try_into((Into::<u64,u128>::into(v_b_xor_v_c) * 2) & BoundedInt::<u64>::max().into()).unwrap());
...
fn blake2b(mut b: Array<u8>) -> Array<u8> {
    ...
    let mut h_0: u64 = (
            (
                (*iv.at(0) ^ 0x01010000) ^ (KEY_LENGTH.into() * 256_u64)
            )
            ^

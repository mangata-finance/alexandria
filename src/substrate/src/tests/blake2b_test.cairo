...
fn test_blake2b() {
    ...
    assert(
        u8_array_eq(res.span(), get_expected_output_hash().span()), 'Hash output not as expected'
    );
}
...
fn test_blake2b_2() {
    ...
    assert(res.len() == 32, 'Incorrect hash length');
    assert(
        u8_array_eq(res.span(), get_expected_output_hash_2().span()), 'Hash output not as expected'
    );
}

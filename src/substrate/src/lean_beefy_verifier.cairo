...
use alexandria_substrate::substrate_storage_read_proof_verifier::{
    convert_u8_subarray_to_felt252_array
};
...
impl PartialEqBeefyPayloadEntryPlan<
    T, impl TEq: PartialEq<T>
> of PartialEq<BeefyPayloadEntryPlan<T>> {
    fn eq(lhs: @BeefyPayloadEntryPlan<T>, rhs: @BeefyPayloadEntryPlan<T>) -> bool {
        let mut eq = true;
        let mut itr: usize = 0;
        loop {
            if itr == BEEFY_PAYLOAD_ID_LEN {
                break;
            }
            if !TEq::eq(
                (*lhs.span).at(itr + *lhs.BeefyPayloadIdPlanStart),
                (*rhs.span).at(itr + *rhs.BeefyPayloadIdPlanStart)
            ) {
                eq = false;
                break;
            }
...
impl PartialOrdBeefyPayloadEntryPlan<
    T, impl TPartialOrd: PartialOrd<T>, impl TCopy: Copy<T>, impl TDrop: Drop<T>
> of PartialOrd<BeefyPayloadEntryPlan<T>> {
    // Big Endian
...
fn decode_paradata(para_data: Span<u8>) -> (u32, Span<u8>) {
    ...
    let para_id = u32_decode(
        Slice { span: para_data, range: Range { start: offset, end: offset + 4 } }
    );
    offset = offset + 4;
...
fn merkelize_row(mut row: Span<u256>, ) -> Result<u256, Array<u256>> {
    ...
    let res: Result<u256, ()> = loop {
        ...
        if a.is_some() && b.is_some() {
            let merge_hash_le: u256 = keccak_u256s_be_inputs(
                array![*a.expect('is_some checked'), *b.expect('is_some checked')].span()
            );
            ...
            if !next.is_empty() {
                next.append(*a.expect('is_some checked'));
            }// Last item = root.
            else {
                break Result::Ok(*a.expect('is_some checked'));
            };
        } else {
...
fn get_hashes_from_items(
    ...
    let maybe_err: Result<(), felt252> = loop {
        if itr == item_lengths_len {
            break Result::Ok(());
        }
        let item_len = *item_lengths.at(itr);

        if offset + item_len > buffer_len {
            break Result::Err('offset beyond buffer len');
        }

        let hash_le: u256 = keccak_le(
            Slice { span: buffer, range: Range { start: offset, end: offset + item_len } }
        );
        let hash: u256 = u256_byte_reverse(hash_le);
...
fn verify_merkle_proof(
    root: u256, proof: Span<u256>, number_of_leaves: usize, leaf_index: usize, leaf_hash: u256, 
) -> bool {
...
fn encoded_leaf_to_leaf(buffer: Span<u8>) -> Result<BeefyData, felt252> {
    ...
    let block_number: u32 = u32_decode(
        Slice { span: buffer, range: Range { start: offset, end: offset + 4 } }
    );
    offset = offset + 4;
    ...
    let next_autority_set_id: u64 = u64_decode(
        Slice { span: buffer, range: Range { start: offset, end: offset + 8 } }
    );
    offset = offset + 8;

    let next_autority_set_len: u32 = u32_decode(
        Slice { span: buffer, range: Range { start: offset, end: offset + 4 } }
    );
    offset = offset + 4;

    let next_autority_set_commitment: u256 = *hashes_to_u256s(buffer.slice(offset, HASH_LENGTH))
        .expect('Should u256')
        .at(0);
    offset = offset + HASH_LENGTH;

    let leaf_extra = *hashes_to_u256s(buffer.slice(offset, HASH_LENGTH))
        .expect('Should u256')
        .at(0);
    offset = offset + HASH_LENGTH;
...
fn encoded_opaque_leaves_to_hashes(buffer: Span<u8>) -> Result<Array<u256>, felt252> {
    ...
    let maybe_err: Result<(), felt252> = loop {
        ...
        let hash_le: u256 = keccak_le(
            Slice { span: buffer, range: Range { start: offset, end: offset + leaf_length } }
        );
        let hash: u256 = u256_byte_reverse(hash_le);
        hashes.append(hash);
        offset = offset + leaf_length;
...
fn verify_mmr_leaves_proof(
    mmr_root: u256, encoded_mmr_leaves_proof: Span<u8>, leaves_hashes_be_u256s: Span<u256>
) -> Result<(), felt252> {
    ...
    loop {
        ...
        leaves
            .append(
                Leaf {
                    pos: leaf_index_to_pos(*leaf_indices.at(itr)),
                    hash: *leaves_hashes_be_u256s.at(itr)
                }
            );
        itr = itr + 1;
    };
    ...
    let calculated_root_be_u256 = calculate_root(
        leaves.span(), mmr_size, proof_items_be_u256.span()
    )?;
...
fn calculate_peaks_hashes(
    mut leaves: Span<Leaf>, mmr_size: u64, mut proof_items: Span<u256>
) -> Result<Array<u256>, felt252> {
    if leaves.is_empty() {
...
fn calculate_peak_root(
    leaves: Span<Leaf>, peak_pos: u64, ref proof_items: Span<u256>
) -> Result<u256, felt252> {
    let mut itr: usize = 0;
    ...
    let result: Result<u256, felt252> = loop {
        match queue.pop_front() {
            Option::Some((
                ...
                if next_height > height {
                    let merge_hash_le: u256 = keccak_u256s_be_inputs(
                        array![sibling_item, item].span()
                    );
                    parent_item = u256_byte_reverse(merge_hash_le);
                } else {
                    let merge_hash_le: u256 = keccak_u256s_be_inputs(
                        array![item, sibling_item].span()
                    );
                    parent_item = u256_byte_reverse(merge_hash_le);
                };
...
fn calculate_root(
    leaves: Span<Leaf>, mmr_size: u64, proof_items: Span<u256>
) -> Result<u256, felt252> {
...
fn u8_eth_addresses_to_u256(u8_addresses: Span<u8>) -> Array<u256> {
    ...
    assert(
        number_of_validators * VALIDATOR_ADDRESS_LEN == u8_addresses.len(),
        'bad validator_addresses len'
    );
    ...
    loop {
        ...
        loop {
            ...
            a_low = Into::<u8, u128>::into(*u8_addresses.at(4 + vitr + itr * VALIDATOR_ADDRESS_LEN))
                + a_low * 256_u128;

            vitr = vitr + 1;
        };

        let a_high: u128 = Into::<u8, u128>::into(*u8_addresses.at(3 + itr * VALIDATOR_ADDRESS_LEN))
            + ((Into::<u8, u128>::into(*u8_addresses.at(2 + itr * VALIDATOR_ADDRESS_LEN))
                + ((Into::<u8, u128>::into(*u8_addresses.at(1 + itr * VALIDATOR_ADDRESS_LEN))
                    + (Into::<u8, u128>::into(*u8_addresses.at(0 + itr * VALIDATOR_ADDRESS_LEN))
                        * 256_u128))
                    * 256_u128))
                * 256_u128);
...
fn get_mmr_root(beefy_payloads: Span<BeefyPayloadEntryPlan<u8>>) -> Result<u256, felt252> {
    ...
    match binary_search(
        beefy_payloads,
        BeefyPayloadEntryPlan {
            span: get_mmr_root_payload_id(),
            BeefyPayloadIdPlanStart: 0_usize,
            BeefyPayloadValuePlan: Range {
                start: BEEFY_PAYLOAD_ID_LEN, end: BEEFY_PAYLOAD_ID_LEN
            },
        }
    ) {
        Option::Some(index) => {
            let beefy_payload: BeefyPayloadEntryPlan<u8> = *beefy_payloads.at(index);
            let beefy_payload_value_len: usize = beefy_payload.BeefyPayloadValuePlan.end
                - beefy_payload.BeefyPayloadValuePlan.start;
                ...
                return Result::Ok(
                    *hashes_to_u256s(
                        beefy_payload
                            .span
                            .slice(
                                beefy_payload.BeefyPayloadValuePlan.start, beefy_payload_value_len
                            )
                    )
        ...
        Option::None => {
            return Result::Err('mmr_root not in beefy_payloads');
        }
    }
}
...
fn get_lean_beefy_proof_metadata(
    buffer: Span<u8>
) -> Result<(BeefyProofMetadata, BeefyProofInfo, Array<BeefyPayloadEntryPlan<u8>>), felt252> {
    let mut offset: usize = 0;
    ...
    let maybe_err: Result<(), felt252> = loop {
        ...
        let beefy_payload_value_plan = Range {
            start: offset, end: offset + beefy_payload_value_len
        };
        ...
        beefy_payloads
            .append(
                BeefyPayloadEntryPlan {
                    span: buffer,
                    BeefyPayloadIdPlanStart: beefy_payload_id_plan,
                    BeefyPayloadValuePlan: beefy_payload_value_plan
                }
            );
    ...
    let block_number = u32_decode(
        Slice { span: buffer, range: Range { start: offset, end: offset + 4 } }
    );
    offset = offset + 4;

    let validator_set_id = u64_decode(
        Slice { span: buffer, range: Range { start: offset, end: offset + 8 } }
    );
    offset = offset + 8;
    ...
    let commitment = Slice {
        span: buffer, range: Range { start: commitment_start, end: commitment_end }
    };
    ...
    let signatures_from_bitfield = Slice {
        span: buffer, range: Range { start: offset, end: offset + signatures_from_bitfield_len }
    };
    ...
    let validator_set_len = u32_decode(
        Slice { span: buffer, range: Range { start: offset, end: offset + 4 } }
    );
    offset = offset + 4;

    let signatures_compact_len = compact_u32_decode(buffer, ref offset)?;
    let signatures_compact = Slice {
        span: buffer, range: Range {
            start: offset, end: offset + (signatures_compact_len * VALIDATOR_SIGNATURE_LEN)
        }
    };
    ...
    let beefy_proof_metadata = BeefyProofMetadata {
        ...
        signatures_from_bitfield: signatures_from_bitfield
            .span
            .slice(
                signatures_from_bitfield.range.start,
                signatures_from_bitfield.range.end - signatures_from_bitfield.range.start
            ),
        validator_set_len: validator_set_len,
        signatures_compact_len: signatures_compact_len,
        signatures_compact: signatures_compact
            .span
            .slice(
                signatures_compact.range.start,
                signatures_compact.range.end - signatures_compact.range.start
            )
    };
...
fn verify_beefy_signatures(
    ...
    signatures_compact_len: u32,
    signatures_compact: Span<u8>,
    validator_addresses: Span<u256>
) -> Result<bool, felt252> {
    let provided_signatures_compact_len = signatures_compact.len() / VALIDATOR_SIGNATURE_LEN;
    assert(
        provided_signatures_compact_len * VALIDATOR_SIGNATURE_LEN == signatures_compact.len(),
        'bad signatures_compact len'
    );
    assert(
        signatures_compact_len == provided_signatures_compact_len, 'signatures_compact len mismatch'
    );
    ...
    let maybe_err: Result<(), felt252> = loop {
        ...
        match move_over_zeros(
            signatures_from_bitfield,
            ref bitfield_byte_pointer,
            ref bitfield_byte_bit_selector,
            ref validator_count
        ) {
            Result::Ok(()) => {},
            Result::Err(e) => {
...
fn move_over_zeros(
    bitfield: Span<u8>,
    ref bitfield_byte_pointer: usize,
    ref selector: u8,
    ref validator_count: usize
) -> Result<(), felt252> {
    // changes to selector and bitfield_pointer must be valid
    loop {
        if (*bitfield.at(bitfield_byte_pointer) & selector) != selector {
            inc_bitfield_marker_unchecked(
                bitfield, ref bitfield_byte_pointer, ref selector, ref validator_count
            );
...
fn inc_bitfield_marker_unchecked(
    bitfield: Span<u8>,
    ref bitfield_byte_pointer: usize,
    ref selector: u8,
    ref validator_count: usize
...
fn keccak_le(input: Slice<u8>) -> u256 {
    ...
    loop {
        ...
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 7))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 6))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 5))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 4))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 3))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 2))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 1))
            + word * 256_u64;
        word = Into::<u8, u64>::into(*input.span.at(input.range.start + itr * 8 + 0))
            + word * 256_u64;
    ...
    loop {
        ...
        last_input_word = Into::<u8, u64>::into(*input.span.at(input.range.end - 1 - itr))
            + last_input_word * 256_u64;
...
fn verify_eth_signature_pre_hashed(
    pre_hashed_message: u256, signature: Span<u8>, address: Span<u8>
) -> bool {
...
fn verify_eth_signature_pre_hashed_u256_address(
    pre_hashed_message: u256, signature: Span<u8>, address: u256
) -> bool {
...
fn u64_decode(slice: Slice<u8>) -> u64 {
    ...
    let res: u64 = (first
        + ((second
            + ((third
                + ((fourth + ((fifth + ((sixth + ((seventh + (eigth * 256)) * 256)) * 256)) * 256))
                    * 256))
                * 256))
            * 256));
    res
